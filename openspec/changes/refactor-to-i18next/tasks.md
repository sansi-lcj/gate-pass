...
- [/] 1.3 **Provider Implementation**: Create `src/components/providers/I18nProvider.tsx` for client-side instance management.
...
- [/] 3.1 **Build Check**: Ensure `npm run build` passes (no `next-intl` remnants).
- [ ] 3.2 **E2E Validation**: Re-run `invitation.spec.ts` to confirm text still renders correctly (functionality unchanged).
- [ ] 3.3 **Manual Check**: Verify `npm run dev` and standard invite flow.

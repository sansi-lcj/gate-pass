    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-950 via-emerald-900 to-teal-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 bg-emerald-950/80 backdrop-blur-lg border border-emerald-600/30 rounded-lg p-8 md:p-12 max-w-2xl w-full shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-emerald-600/30">
          ...
          <span className="text-amber-400 font-serif tracking-widest">POINCARÉ</span>
        </div>
        ...
        <div className="bg-emerald-900/50 border border-emerald-600/30 rounded-lg p-6 my-8">
          <p className="text-amber-400/80 text-sm">{t('Invitation.event_time_label')}</p>
          <p className="text-2xl text-white font-medium mt-1">{localTime || '...'}</p>
          <p className="text-emerald-500 text-xs">{t('Invitation.event_time_local')}</p>
          <div className="flex flex-wrap gap-3 mt-6">
            <span className="bg-emerald-800/50 text-emerald-300 px-3 py-1 rounded border border-emerald-600/30 text-sm">✦ {t('Invitation.highlights.item1')}</span>
            <span className="bg-emerald-800/50 text-emerald-300 px-3 py-1 rounded border border-emerald-600/30 text-sm">✦ {t('Invitation.highlights.item2')}</span>
            <span className="bg-emerald-800/50 text-emerald-300 px-3 py-1 rounded border border-emerald-600/30 text-sm">✦ {t('Invitation.highlights.item3')}</span>
          </div>
        </div>

        {eventEnded && <div className="bg-emerald-900/50 text-emerald-400 p-4 rounded-lg mb-6 text-center">{t('Invitation.event_ended')}</div>}
        ...
        {status === 'ACCEPTED' && data.discountCode && (
          <div className="bg-gradient-to-r from-amber-900/50 to-amber-800/50 border border-amber-500/30 rounded-lg p-8 mb-8 text-center">
            <p className="text-amber-400/70 text-sm">{t('Invitation.code_label')}</p>
            <p className="text-4xl font-bold text-amber-300 tracking-widest mt-2">{data.discountCode}</p>
            {data.meetingLink && <a href={data.meetingLink} target="_blank" rel="noopener noreferrer" className="text-amber-400 underline mt-4 inline-block">{t('Invitation.meeting_link_label')}</a>}
          </div>
        )}
        ...
        {status === 'DECLINED' && (
          <div className="bg-red-900/30 border border-red-500/30 rounded-lg p-6 mb-8 text-center">
            ...
            <p className="text-emerald-400/70 text-sm">{t('Invitation.declined_desc')}</p>
            {!eventEnded && <button onClick={handleReconsider} disabled={loading} className="text-amber-400 underline mt-4">{t('Invitation.reconsider_btn')}</button>}
          </div>
        )}

        {(status === 'PENDING' || status === 'OPENED') && !eventEnded && (
          <div className="flex flex-col md:flex-row gap-4 justify-center mt-8">
            <button onClick={handleAccept} disabled={loading} className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-emerald-950 font-bold py-4 px-10 rounded transition-all disabled:opacity-50">
              {t('Invitation.accept_btn')}
            </button>
            <button onClick={handleDecline} disabled={loading} className="border border-emerald-600/50 text-emerald-400 hover:border-red-500/50 hover:text-red-400 py-4 px-10 rounded transition-all disabled:opacity-50">
        ...
        {status === 'ACCEPTED' && (
          <div className="text-center mt-4">
            <span className="inline-block bg-green-900/50 text-green-400 border border-green-500/30 px-6 py-3 rounded">✓ {t('Invitation.accepted_desc')}</span>
          </div>
        )}

        <div className="mt-12 pt-6 border-t border-emerald-600/30 text-center">
          <p className="text-emerald-600 text-xs">{t('Invitation.footer')} © 2025</p>
        </div>
      </div>
    </div>
  );

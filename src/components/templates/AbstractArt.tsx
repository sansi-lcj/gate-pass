    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-100 via-violet-100 to-teal-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 bg-white/80 backdrop-blur-xl rounded-3xl p-8 md:p-12 max-w-2xl w-full shadow-xl">
        ...
        <div className="flex items-center justify-center gap-4 mb-8">
          ...
          <span className="text-violet-600 text-xs bg-violet-100 px-3 py-1 rounded-full">POINCARÉ</span>
        </div>
        ...
        <div className="bg-gradient-to-r from-rose-50 via-violet-50 to-teal-50 rounded-2xl p-6 my-8 text-center">
          <p className="text-violet-600 text-sm">{t('Invitation.event_time_label')}</p>
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-6">
            <span className="bg-rose-200/50 text-rose-700 px-4 py-2 rounded-full text-sm">{t('Invitation.highlights.item1')}</span>
            <span className="bg-violet-200/50 text-violet-700 px-4 py-2 rounded-full text-sm">{t('Invitation.highlights.item2')}</span>
            <span className="bg-teal-200/50 text-teal-700 px-4 py-2 rounded-full text-sm">{t('Invitation.highlights.item3')}</span>
          </div>
        </div>
        ...
        {status === 'ACCEPTED' && data.discountCode && (
          <div className="bg-gradient-to-r from-rose-500 via-violet-500 to-teal-500 text-white rounded-2xl p-8 mb-8 text-center">
            <p className="text-white/80 text-sm">{t('Invitation.code_label')}</p>
            <p className="text-4xl font-bold tracking-widest mt-2">{data.discountCode}</p>
            {data.meetingLink && <a href={data.meetingLink} target="_blank" rel="noopener noreferrer" className="text-white/80 underline mt-4 inline-block">{t('Invitation.meeting_link_label')}</a>}
          </div>
        )}
        ...
        {status === 'DECLINED' && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 mb-8 text-center">
            ...
            {!eventEnded && <button onClick={handleReconsider} disabled={loading} className="text-violet-500 underline mt-4">{t('Invitation.reconsider_btn')}</button>}
          </div>
        )}

        {(status === 'PENDING' || status === 'OPENED') && !eventEnded && (
          <div className="flex flex-col md:flex-row gap-4 justify-center mt-8">
            <button onClick={handleAccept} disabled={loading} className="bg-gradient-to-r from-rose-500 via-violet-500 to-teal-500 hover:opacity-90 text-white font-bold py-4 px-10 rounded-full transition-all shadow-lg disabled:opacity-50">

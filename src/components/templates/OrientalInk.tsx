...
export default function OrientalInk({ data }: InvitationProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-stone-100 to-stone-200 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 bg-white/90 backdrop-blur p-8 md:p-12 max-w-2xl w-full border border-stone-300" style={{ boxShadow: '8px 8px 0 rgba(120, 113, 108, 0.2)' }}>
        ...
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-stone-300">
          ...
          <div className="text-right">
            <span className="text-stone-500 text-xs tracking-widest">POINCARÉ</span>
            <p className="text-stone-800 text-lg font-serif">如影随形</p>
          </div>
        </div>
        ...
        <div className="border border-stone-300 bg-stone-50 p-6 my-8">
          <p className="text-stone-500 text-sm">{t('Invitation.event_time_label')}</p>
          <p className="text-2xl text-stone-800 font-serif mt-1">{localTime || '...'}</p>
          <p className="text-stone-400 text-xs">{t('Invitation.event_time_local')}</p>
          <div className="mt-6 space-y-2 text-stone-600">
            <p className="flex items-center gap-2"><span className="text-stone-400">●</span> {t('Invitation.highlights.item1')}</p>
            <p className="flex items-center gap-2"><span className="text-stone-400">●</span> {t('Invitation.highlights.item2')}</p>
            <p className="flex items-center gap-2"><span className="text-stone-400">●</span> {t('Invitation.highlights.item3')}</p>
          </div>
        </div>

        {eventEnded && <div className="bg-stone-100 text-stone-500 p-4 mb-6 text-center">{t('Invitation.event_ended')}</div>}

        {status === 'ACCEPTED' && data.discountCode && (
          <div className="bg-stone-800 text-white p-8 mb-8 text-center">
            <p className="text-stone-400 text-sm">{t('Invitation.code_label')}</p>
            <p className="text-3xl font-serif tracking-widest mt-2">{data.discountCode}</p>
            {data.meetingLink && <a href={data.meetingLink} target="_blank" rel="noopener noreferrer" className="text-stone-400 underline mt-4 inline-block">{t('Invitation.meeting_link_label')}</a>}
          </div>
        )}
        ...
        {status === 'DECLINED' && (
          <div className="bg-red-50 border border-red-200 p-6 mb-8 text-center">
            ...
            <p className="text-stone-500 text-sm">{t('Invitation.declined_desc')}</p>
            {!eventEnded && <button onClick={handleReconsider} disabled={loading} className="text-stone-600 underline mt-4">{t('Invitation.reconsider_btn')}</button>}
          </div>
        )}

        {(status === 'PENDING' || status === 'OPENED') && !eventEnded && (
          <div className="flex flex-col md:flex-row gap-4 justify-center mt-8">
            <button onClick={handleAccept} disabled={loading} className="bg-stone-800 hover:bg-stone-700 text-white py-4 px-10 transition-all disabled:opacity-50">
              {t('Invitation.accept_btn')}
            </button>
            <button onClick={handleDecline} disabled={loading} className="border border-stone-400 text-stone-500 hover:border-red-400 hover:text-red-500 py-4 px-10 transition-all disabled:opacity-50">
        ...
        {status === 'ACCEPTED' && (
          <div className="text-center mt-4">
            <span className="inline-block bg-green-50 text-green-700 border border-green-200 px-6 py-3">✓ {t('Invitation.accepted_desc')}</span>
          </div>
        )}

        <div className="mt-12 pt-6 border-t border-stone-300 text-center">
          <p className="text-stone-400 text-xs">{t('Invitation.footer')} © 2025</p>
        </div>
      </div>
    </div>
  );
